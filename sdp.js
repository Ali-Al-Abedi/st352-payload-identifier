...
/** SSM source must be unicast IPv4 (not multicast / not blank). */
export function isUnicastV4(s) {
  return isIPv4(s) && !isMulticastV4(s);
}
...
export function validateSpec(rawSpec) {
  ...
  if (!nonEmptyStr(spec.origin.user)) errors.push('origin user is required');
  ...
  if (spec.redundant) {
    const [ga, gb] = spec.groupLabels || [];
    if (!nonEmptyStr(ga) || !nonEmptyStr(gb)) {
      errors.push('ST 2022-7 group labels (a=group / a=mid) are required');
    }
  }
  ...
  spec.legs.slice(0, legCount).forEach((leg, i) => {
    ...
    // Source is optional (ASM); when present it must be unicast SSM.
    if (hasSource(leg)) {
      if (!isIPv4(leg.source)) {
        errors.push(`${tag} leg source "${leg.source}" is not a valid IPv4 address`);
      } else if (!isUnicastV4(leg.source)) {
        errors.push(`${tag} leg source "${leg.source}" must be unicast (not multicast)`);
      }
    }
  });
  ...
  if (spec.type === 'video') {
    ...
    for (const [label, val] of [
      ...
      if (!nonEmptyStr(val)) errors.push(`video ${label} is required`);
    }
  } else if (spec.type === 'audio') {
    const a = spec.audio;
    ...
    if (!nonEmptyStr(a.channelOrder)) errors.push('audio channel-order is required');
  } else if (spec.type === 'anc') {
    const n = spec.anc;
